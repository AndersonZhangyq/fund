...
if __name__ == "__main__":
    ...
    parser.add_argument("--duration", type=int, default=30, help="回测天数，默认30天")
    parser.add_argument("--file", type=str, default=None, help="组合中基金的代码，基金代码之间用逗号隔开")
    parser.add_argument("--force_update", action="store_true", help="强制更新本地数据，默认状态下如果今天获取过则不再获取")
    ...
    if args.file is None:
        print(f"未提供基金代码文件，将使用默认值 {code_list}")
    else:
        with open(args.file) as f:
            line = f.readline()
            code_list = [i.strip() for i in line.split(',')]
